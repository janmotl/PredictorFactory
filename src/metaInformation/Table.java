...
//	}
	...
	public Column getColumn(String columnName) {
		...
		if (!columnMap.containsKey(columnName)) throw new NullPointerException("The column " + columnName + " was not found in table " + name);
	...
	public void categorizeColumns(Setting setting) {
		...
		if (columnMap == null || columnMap.isEmpty()) {
			Table.logger.info("Table: " + name + " doesn't contain any column beside ids.");
			return;
		}
		...
		for (Column column : columnMap.values()) {
			...
			if (dataType == -16 || dataType == -15 || dataType == -9 || dataType == -1 || dataType == 1 || dataType == 12) {
				...
				columnCounter++;
				Table.logger.debug("Ignoring column: " + name + "." + column.name
		...
		if (columnCounter > 0) {
			Table.logger.warn("In total " + columnCounter + " columns were ignored in " + name + " because of unsupported data type.");
		}
